...
public class SongAction extends Action {
	...
	MyPagination<SongForm> pagination = null;
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		} else if ("vip_search".equals(action)) {
			return vip_search(mapping, form, request, response); // 后台查询歌曲信息
		}else
	...
	private ActionForward vip_search(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) {
		// TODO Auto-generated method stub
		String strPage = (String) request.getParameter("Page");
		String name=(String) request.getSession().getAttribute("vipname");
		String condition = "";
		int songType = 0;
		String fieldName_cn = "";
		String typeName = "";
		...
		if (strPage == null) {
			...
			pagination = (MyPagination<SongForm>) request.getSession().getAttribute(
					"pagination");
		...
		request.getSession().setAttribute("vipname", name);
		...
		return mapping.findForward("vip_search");
	}
	...
	public ActionForward main(ActionMapping mapping, ActionForm form,
		...
		for (int i = 0; i < list.size(); i++) {
			...
			request.setAttribute(
					"newSongList" + i,
					songDAO.query("WHERE songType=" + songTypeId
	...
	public ActionForward songSort(ActionMapping mapping, ActionForm form,
		...
		if ("hits".equals(type)) {
			request.setAttribute("sortType",
					songDAO.query(" ORDER BY hits DESC", 8)); // 获取试听排行信息
		} else if ("download".equals(type)) {
			request.setAttribute("sortType",
					songDAO.query(" ORDER BY download DESC", 8)); // 获取下载排行信息
		}
	...
	public ActionForward search(ActionMapping mapping, ActionForm form,
		...
		List<SongForm> list = null;
		if (strPage == null) {
			pagination = new MyPagination<SongForm>();
			list = songDAO.query(condition); // 获取歌曲信息
	...
	public ActionForward tryListen(ActionMapping mapping, ActionForm form,
		...
		String mp3RealPath = lrcRealPath.substring(0,
				lrcRealPath.lastIndexOf("/") + 1)
				+ "music/" + urlAndName[1];
		request.setAttribute("realPath", mp3RealPath);
		lrcRealPath = lrcRealPath
				+ "music/"
				+ urlAndName[1]
						.substring(0, urlAndName[1].lastIndexOf(".") + 1)
				+ "lrc"; // lrc文件路径
	...
	public ActionForward continuePlay(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute("songNameList",
				songDAO.continuePlay(playID, url, "ORDER BY upTime DESC")); // 连续播放
	...
	public ActionForward download(ActionMapping mapping, ActionForm form,
		 ...
		 * request.setAttribute
		 * ("error","请不要盗链本网站的资源，请访问http://www.mrbccd.com进行下载！"); return
		 * mapping.findForward("error"); }else{
		 */
		/**************************************************************/
		int id = Integer.parseInt(request.getParameter("id"));
		String fileURL = songDAO.getFileURL(id);
		// System.out.println("文件绝对地址：" + request.getRealPath("/music/" +
		// fileURL));
	...
	public ActionForward del(ActionMapping mapping, ActionForm form,
		...
		String lrcFileURL = fileURL.substring(0, fileURL.lastIndexOf("."))
				+ ".lrc"; // 组成歌词文件的名称
		// 删除数据
